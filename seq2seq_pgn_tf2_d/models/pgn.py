...
from seq2seq_pgn_tf2_d.encoders import encoder
from seq2seq_pgn_tf2_d.attentions import attention
from seq2seq_pgn_tf2_d.decoders import decoder
from seq2seq_pgn_tf2_d.attentions import pointer
from seq2seq_pgn_tf2_d.utils.data_utils import load_word2vec
...
class PGN(tf.keras.Model):
    ...
    def call(self, dec_hidden, enc_output, dec_inp,
        ...
        # 此处dec_hidden实质为enc_hidden（train_helper.py中dec_hidden = enc_hidden）
        ...
        for t in range(dec_inp.shape[1]):
            ...
            # 这里和上面的那个attention有什么区别,上面只是做了一次初始化,为了拿到encoder出来的context_vector
            ...
            attentions.append(tf.squeeze(attn, axis=2))     # 去掉维度
...
def _calc_final_dist(_enc_batch_extend_vocab, vocab_dists, attn_dists, p_gens, batch_oov_len, vocab_size, batch_size):
    ...
    # vocab_dists的尺寸为（batch_size, vsize）,这里的拼接只是在vsize的大小上做了延长,维度没有变化
    vocab_dists_extended = [tf.concat(axis=1, values=[dist, extra_zeros]) for dist in vocab_dists]

    # tensorflow api：https://www.w3cschool.cn/tensorflow_python/tensorflow_python-led42j40.html
