...
from seq2seq_pgn_tf2_lj.utils.config import vocab_path, train_y_seg_path, train_x_seg_path, sample_total
from seq2seq_pgn_tf2_lj.utils.data_loader import load_train_dataset, load_test_dataset
import tensorflow as tf
import os
from seq2seq_pgn_tf2_lj.utils.file_utils import save_pickle, load_pickle
...
def article_to_ids(article_words, vocab):
    ...
    for w in article_words:
        ...
        if i == unk_id:  # If w is OOV, OOV词的id为unk_id
            if w not in oovs:  # Add to list of OOVs,对oov词进行编码
                oovs.append(w)
...
def abstract_to_ids(abstract_words, vocab, article_oovs):
    '''
    :param abstract_words: 输出（真实标签）
    :param vocab: 输入（词表）
    :param article_oovs: 输入（oov词表）
    :return:
    '''
    ids = []
    unk_id = vocab.word_to_id(Vocab.UNKNOWN_TOKEN)
...
def get_dec_inp_targ_seqs(sequence, max_len, start_id, stop_id):
    ...
    target = sequence[:]        # target不需要开始符
    if len(inp) > max_len:  # truncate
        inp = inp[:max_len]
        target = target[:max_len]  # no end_token
    elif len(inp) == max_len:
        target.append(stop_id)
        # inp.append(stop_id)       # inp也应该加上结束符
    else:
...
def example_generator(params, vocab, max_enc_len, max_dec_len, mode, batch_size):
    if mode == "train" or mode == 'eval':
        ...
        for raw_record in train_dataset:
            ...
            enc_input = [vocab.word_to_id(w) for w in article_words]    #word2id
